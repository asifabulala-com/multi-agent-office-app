...
Three ready-to-run telecom project inputs are included, each with a pre-generated output JSON, MVP HTML dashboard, and HTML report so judges can evaluate the full artifact set without running the system.

### Artifact Map

| # | Input | Output JSON | MVP Dashboard | HTML Report |
|---|---|---|---|---|
| 1 | [input_examples/input_1.json](input_examples/input_1.json) | [output_IT_ServiceDesk_001.json](output_examples/output_IT_ServiceDesk_001.json) | [IT_ServiceDesk_001/mvp.html](output_examples/IT_ServiceDesk_001/mvp.html) | [report_IT_Servi.html](reports/report_IT_Servi.html) |
| 2 | [input_examples/input_2.json](input_examples/input_2.json) | [output_RA_Dashboard_001.json](output_examples/output_RA_Dashboard_001.json) | [RA_Dashboard_001/mvp.html](output_examples/RA_Dashboard_001/mvp.html) | [report_RA_Dashb.html](reports/report_RA_Dashb.html) |
| 3 | [input_examples/input_3.json](input_examples/input_3.json) | [output_NOC_Monitor_001.json](output_examples/output_NOC_Monitor_001.json) | [NOC_Monitor_001/mvp.html](output_examples/NOC_Monitor_001/mvp.html) | [report_NOC_Moni.html](reports/report_NOC_Moni.html) |

---

### Example 1 — IT Service Management Portal

**Input:** [`input_examples/input_1.json`](input_examples/input_1.json)

```json
{
  "project_id": "IT_ServiceDesk_001",
  "project_name": "IT Service Management Portal",
  "description": "Build an internal IT Service Management (ITSM) portal for a telecom company's Corporate IT department. The system must support incident ticketing (create, assign, escalate, resolve), change request management with approval workflows, a self-service knowledge base, SLA tracking with breach alerts, and an asset inventory linked to tickets. Use Django REST Framework and Python 3.11 for the backend, Vue 3 and TypeScript for the frontend, and PostgreSQL for the database. Implement three user roles: end-user, IT technician, and IT manager. Send email notifications on ticket state changes. Include a dashboard showing open tickets by priority, SLA compliance rate, and mean time to resolution (MTTR) by category.",
  "use_case_id": "1",
  "input": "Build an IT Service Management portal with incident ticketing, knowledge base, SLA tracking, and asset inventory for a telecom company",
  "context": {
    "domain": "telecom",
...
**What the agents did:**
- Risk Analyst detected **high-severity risks** (SLA breach liability, multi-role auth complexity) → triggered PM replan before engineering
- All **4 collaboration patterns** fired: `risk_escalation_to_pm`, `stakeholder_approval_with_conditions`, `qa_critique_and_revision_loop`, `replan_based_on_feedback`
- QA critique-and-revision loop ran on engineering tasks → Engineer revised implementations based on structured QA findings

**Output artifacts:**

| Artifact | File | Key contents |
|---|---|---|
| **Output JSON** | [`output_examples/output_IT_ServiceDesk_001.json`](output_examples/output_IT_ServiceDesk_001.json) | `risk_level: "high"` · `onet_resource_risk: "medium"` · `nist_cves_found: 6` · `iterations: 2` · `trace_events: 245` |
| **MVP Dashboard** | [`output_examples/IT_ServiceDesk_001/mvp.html`](output_examples/IT_ServiceDesk_001/mvp.html) | Self-contained HTML app generated by Engineer — open in any browser, zero dependencies |
| **HTML Report** | [`reports/report_IT_Servi.html`](reports/report_IT_Servi.html) | Mermaid agent sequence diagram · PM task cards · risk register · inter-agent message log · QA feedback loops |

---

### Example 2 — Revenue Assurance Dashboard

**Input:** [`input_examples/input_2.json`](input_examples/input_2.json)

```json
{
  "project_id": "RA_Dashboard_001",
  "project_name": "Revenue Assurance Dashboard",
  "description": "Build a web-based Revenue Assurance Dashboard for a telecom company's finance team. The system must ingest daily CDR (Call Detail Records) and billing feeds, detect revenue leakage through automated reconciliation rules, flag discrepancies between network usage and billed amounts, and produce drill-down reports by region, product, and time period. Use FastAPI and Python 3.11 for the backend, React 18 and TypeScript for the frontend, PostgreSQL for structured data, and Redis for caching reconciliation results. Include role-based access for analysts, managers, and auditors. The dashboard must display KPIs for revenue at risk, recovery rate, and open leakage cases.",
  "use_case_id": "2",
  "input": "Build a Revenue Assurance Dashboard for CDR reconciliation and revenue leakage detection in a telecom finance team",
  "context": {
    "domain": "telecom",
    "department": "finance",
...
**What the agents did:**
- Risk Analyst flagged **CVE-level security risks** for the FastAPI + PostgreSQL stack (NIST NVD: 6 CVEs) → injected into QA critique context
- Stakeholder applied conditional approval → PM adjusted plan before engineering began
- All **4 collaboration patterns** fired: `risk_escalation_to_pm`, `stakeholder_approval_with_conditions`, `qa_critique_and_revision_loop`, `replan_based_on_feedback`
...
| Artifact | File | Key contents |
|---|---|---|
| **Output JSON** | [`output_examples/output_RA_Dashboard_001.json`](output_examples/output_RA_Dashboard_001.json) | `risk_level: "high"` · `onet_resource_risk: "medium"` · `nist_cves_found: 6` · `iterations: 2` · `trace_events: 115` |
| **MVP Dashboard** | [`output_examples/RA_Dashboard_001/mvp.html`](output_examples/RA_Dashboard_001/mvp.html) | Self-contained HTML dashboard app — revenue KPIs, leakage case list, reconciliation status |
| **HTML Report** | [`reports/report_RA_Dashb.html`](reports/report_RA_Dashb.html) | Agent timeline · risk register with NIST CVE citations · stakeholder conditional approval record |

---

### Example 3 — NOC Monitoring Platform

**Input:** [`input_examples/input_3.json`](input_examples/input_3.json)

```json
{
  "project_id": "NOC_Monitor_001",
  "project_name": "Network Operations Center Monitoring Platform",
  "description": "Build a real-time Network Operations Center (NOC) monitoring platform for a telecom operator's network engineering team. The system must ingest SNMP traps, syslog streams, and NetFlow data from 500+ network devices (routers, switches, base stations), correlate events into incidents using rule-based and ML-assisted engines, display a live topology map with device health status, and escalate P1/P2 outages via PagerDuty and SMS. Use FastAPI and Python 3.11 for the backend event processor, React 18 with WebSockets for the live dashboard, InfluxDB for time-series metrics, and PostgreSQL for incident management. Implement three alert severity levels (P1 critical, P2 major, P3 minor) with auto-escalation rules. Include an SLA reporting module showing MTTR, MTTA, and availability by region and technology domain.",
  "use_case_id": "1",
  "input": "Build a real-time NOC monitoring platform for SNMP/syslog ingestion, event correlation, live topology map, and P1/P2 auto-escalation for a telecom network operations team",
  "context": {
    "domain": "telecom",
    "department": "network-operations",
    "audience": "noc-engineers-and-network-managers"
  }
}
```

**What the agents did:**
- Complexity of real-time WebSocket + InfluxDB + 500-device scale triggered **risk escalation** → PM replanned task ordering
- QA flagged missing alert-threshold unit tests → Engineer revised and resubmitted
- **2 collaboration patterns** fired: `risk_escalation_to_pm`, `qa_critique_and_revision_loop`

**Output artifacts:**

| Artifact | File | Key contents |
|---|---|---|
| **Output JSON** | [`output_examples/output_NOC_Monitor_001.json`](output_examples/output_NOC_Monitor_001.json) | `risk_level: "high"` · `onet_resource_risk: "medium"` · `nist_cves_found: 6` · `iterations: 2` · `trace_events: 35` |
| **MVP Dashboard** | [`output_examples/NOC_Monitor_001/mvp.html`](output_examples/NOC_Monitor_001/mvp.html) | Self-contained NOC dashboard — device health grid, alert severity panel, incident log |
| **HTML Report** | [`reports/report_NOC_Moni.html`](reports/report_NOC_Moni.html) | Agent collaboration timeline · risk register · QA critique + revision record |

---

### Output JSON — RunResponse Format

All three output JSONs follow the same `RunResponse` schema returned by `POST /run`:

```json
{
  "status": "success",
  "request_id": "IT_ServiceDesk_001",
  "execution_time_seconds": 47.3,
  "output": {
    "project_id":            "IT_ServiceDesk_001",
    "workflow_status":       "completed",
    "result_summary":        "ITSM portal planned, risk-assessed, implemented...",
    "confidence":            0.87,
    "risk_level":            "high",
    "onet_resource_risk":    "medium",
    "nist_cves_found":       6,
    "bls_estimated_cost_usd": 0,
    "mvp_path":              "output_examples/IT_ServiceDesk_001/mvp.html",
    "iterations":            2,
    "interaction_counts":    { "plan": 3, "implement": 4, "qa": 6, "risk": 5 }
  },
  "metadata": {
    "compass_models":        { "reasoning": "gpt-5.1", "standard": "gpt-4.1" },
    "agents_invoked":        ["project_manager","risk_analyst","stakeholder","engineer","qa"],
    "trace_events":          245,
    "real_data_citations":   ["onet_soc_codes","nist_nvd_cves","bls_wage_survey"],
    "collaboration_patterns":["risk_escalation_to_pm","qa_critique_and_revision_loop",
                              "stakeholder_approval_with_conditions","replan_based_on_feedback"],
...
### Serving the Artifacts Live

With the server running, all artifacts are accessible via browser:

```
GET /mvp/IT_ServiceDesk_001    → output_examples/IT_ServiceDesk_001/mvp.html
GET /mvp/RA_Dashboard_001      → output_examples/RA_Dashboard_001/mvp.html
GET /mvp/NOC_Monitor_001       → output_examples/NOC_Monitor_001/mvp.html

GET /report/IT_ServiceDesk_001 → reports/report_IT_Servi.html
GET /report/RA_Dashboard_001   → reports/report_RA_Dashb.html
GET /report/NOC_Monitor_001    → reports/report_NOC_Moni.html
```

The web UI at `http://localhost:8000` shows **Open MVP →** and **Open Report →** buttons after every run. The `dataset/telecom_projects.json` file contains 20 additional pre-configured project descriptions available through the dataset browser in the web UI.
