...
### Maximum 2 engineer tasks per run
`MAX_ITERATIONS = 2` in `app/graph.py`. If the PM produces more than 2 engineer tasks, only the first 2 are processed. This bounds execution time under the 15-minute evaluation cap at the cost of partial project coverage.
...
The `Dockerfile` and `docker-compose.yml` target Python 3.11-slim. This is intentional for image-size reasons. If running locally without Docker, any Python ≥ 3.10 should work.
