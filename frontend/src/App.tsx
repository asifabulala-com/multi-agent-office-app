...
function App() {
  ...
  const [description, setDescription] = useState(
    "Simulate multi-agent planning, execution, QA and risk management."
  );
  ...
  const handleRun = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      const response = await fetch("/run-sync", {
        ...
        body: JSON.stringify({ project_id: projectId, project_name: projectName, description }),
      });
  ...
  const summaryText = useMemo(() => {
    ...
    return `Project ${execution.project_id} completed with status ${execution.status} after ${execution.iterations} iteration(s). Compass evaluation: ${execution.compass_evaluation?.status ?? "unknown"}`;
  }, [execution, error]);
    ...
    <div className="app-shell">
      <div className="header-row">
        <div>
          ...
          <p>Submit a project, inspect execution, and visualize collaboration results.</p>
        </div>
      ...
      <section className="panel">
        ...
        {execution && (
          <details>
            <summary>Show full execution JSON</summary>
